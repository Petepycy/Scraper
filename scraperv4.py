...
class CompanyRegistryExtractor:
    ...
    def extract_contact_email(self, section):
        try:
            email_div = section.find('div', class_='e-mail')
            if email_div:
                email_span = email_div.find('span', class_='questo-paywall')
                if email_span:
                    return email_span.text.strip()
            return None
    ...
    def get_adress_from_overview(self, section):
        try:
            address = section.find("div", class_="address-data").text.strip()
            return address
        except AttributeError:
            return None
    ...
    def extract_categories(self, section):
        try:
            categories = section.find_all('a', class_='category-label')
    ...
    def extract_all(self):
        ...
        overview_section = self.soup.find("app-company-info-section")
        category_section = self.soup.find("app-category-list")
        categories = self.extract_categories(category_section)[:3]
        registry_details = self.get_registry_details(section)

        data = {
            "Nazwa pełna": self.get_full_name(section),
            "Adres siedziby rejestrowe": self.get_address(section),
            "Adres siedziby overview": self.get_adress_from_overview(overview_section),
            ...
            "Kategoria 1": categories[0] if len(categories) > 0 else None,
            "Kategoria 2": categories[1] if len(categories) > 1 else None,
            "Kategoria 3": categories[2] if len(categories) > 2 else None
        }
